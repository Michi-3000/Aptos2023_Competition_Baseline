...
class ResNet_CSRA(ResNet):
    ...
    def forward_train_weighted(self, x, target, weight):
        x = self.backbone(x)
        logit = self.classifier(x)
        loss = self.loss_func(logit, target, reduction="none")
        #print(loss)
        #print(weight)
    ...
    def forward(self, x, target=None, weight=None):
        if target is not None:
            if weight is not None:
                return self.forward_train_weighted(x, target, weight)
            else:
...
class ResNet_CSRA_multi(ResNet):
    arch_settings = {
        18: (BasicBlock, (2, 2, 2, 2)),
        34: (BasicBlock, (3, 4, 6, 3)),
        50: (Bottleneck, (3, 4, 6, 3)),
    ...
    def __init__(self, num_heads, lam, num_classes, depth=101, input_dim=2048, cutmix=None):
        self.block, self.layers = self.arch_settings[depth]
        self.depth = depth
        super(ResNet_CSRA_multi, self).__init__(self.block, self.layers)
        self.init_weights(pretrained=True, cutmix=cutmix)
    ...
    def forward_train(self, x, target):
        #print(x.shape)
        nonZero = x.abs().sum(dim=(0,2,3,4)).bool()
        x=x[:,nonZero,:,:,:]
        B,N,C,W,H = x.size()
        x_embed = self.backbone(x.view(-1,C,W,H)) #(B*N,)    
        T,W,H =  x_embed.size()
        x_out = x_embed.view(B,N,W,H).mean(dim=1)
        print(x_out.shape)
        #x = self.backbone(x)
        logit = self.classifier(x_out)
        loss = self.loss_func(logit, target, reduction="mean")
        return logit, loss
    ...
    def forward_train_weighted(self, x, target, weight):
        ...
        x_embed = self.backbone(x.view(-1,C,W,H)) #(B*N,)    
        #print(x_embed.size())
        T, C, W, H =  x_embed.size()
        x_out = x_embed.view(B,N,C,W,H).mean(dim=1)
        #print(x_out.shape)
        #x = self.backbone(x)
        logit = self.classifier(x_out)
        loss = self.loss_func(logit, target, reduction="none")
